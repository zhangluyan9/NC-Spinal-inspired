...
def single_run(run_id, args, device):
    ...
    # Load data
    train_data = torch.load('train_data_3dimension_temper_rebuttal_nonsrapsi.pt')
    val_data = torch.load('val_data_3dimension_temper_rebuttal_nonsrapsi.pt')
    test_data = torch.load('test_data_3dimension_temper_rebuttal_nonsrapsi.pt')
    ...
    # Training tracking
    best_val_acc = 0.0
    best_model_path = f"snn_model_run_directinput_nonsparse_{run_id}.pt"
    train_losses = []
    val_accuracies = []
    test_accuracies = []
