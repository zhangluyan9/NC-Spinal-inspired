...
for i in range(X.shape[0]):
    ...
    #print(i,label)
    X_expanded[i, :, 0] = 10000 # eucl force 1
    X_expanded[i, :, 1] = 15000  # finger1_pos 
    X_expanded[i, :, 2] = temps[:, 0]

    X_expanded[i, :, 3] = 10000 # eucl force 2
    X_expanded[i, :, 4] = 15000  # finger2_pos
    X_expanded[i, :, 5] = temps[:, 1]
    

    X_expanded[i, :, 6] = 10000 # eucl force 3
    X_expanded[i, :, 7] = 15000 # finger3_pos
    X_expanded[i, :, 8] = temps[:, 2]

    # if label == 8:
    #     print(i)
    #     print(X_expanded[i, :, 2].shape, temp_08[:,i-800].shape)
    #     X_expanded[i, :, 2] = temp_08[:,i-800]
    #     X_expanded[i, :, 5] = temp_08[:,i-800]
    #     X_expanded[i, :, 8] = temp_08[:,i-800]


    # if label == 16:
    #     X_expanded[i, :, 2] = temp_10[:,i-1600]
    #     X_expanded[i, :, 5] = temp_10[:,i-1600]
    #     X_expanded[i, :, 8] = temp_10[:,i-1600]


print(X_expanded.shape)
#print(X_expanded[800])
# Save to new file
np.savez('robot_data_expanded_with_temp_temper.npz', X=X_expanded, y=y)
